...
const defaultSeller = {
  ...
  phone: "094427 50948 / 094425 93708",
  accountName: "Vijayasamundeswari.G",
  bankName: "STATE BANK OF INDIA",
  accNo: "33184314150",
  ifsc: "SBIN0002241",
  branch: "Neikkarapatti Branch",
};
...
function DispatchNote({ initialInvoice, sellerProfile, clients, savedInvoices, onSave, onPrint }) {
  const [seller] = useState({ ...defaultSeller, ...sellerProfile, stateCode: sellerProfile?.state_code || "", stateName: sellerProfile?.state_name || "", accountName: sellerProfile?.account_name || "Vijayasamundeswari.G", bankName: sellerProfile?.bank_name || "STATE BANK OF INDIA" });
...
export default function InvoiceBill({ initialBuyer, initialRows, initialInvoice, sellerProfile, clients = [], savedInvoices = [], itemCatalog = [], onSave, onPrint, onExportPdf }) {
  ...
  const [seller, setSeller] = useState(sellerProfile ? { ...defaultSeller, ...sellerProfile, stateCode: sellerProfile.state_code, stateName: sellerProfile.state_name, accountName: sellerProfile.account_name || "Vijayasamundeswari.G", bankName: sellerProfile.bank_name, accNo: sellerProfile.acc_no } : defaultSeller);
          ...
          <div className="ib-bank">
            ...
            <div className="ib-bank-row">
              <label>Account name</label>
                  <input aria-label="Bank account name" value={seller.accountName || "Vijayasamundeswari.G"} onChange={(e) => setSeller({ ...seller, accountName: e.target.value })} />
            </div>
            <div className="ib-bank-row">
              <label>A/c No.</label>
            ...
            <div className="ib-bank-row">
              <label>Bank name</label>
                  <input aria-label="Bank name" value={seller.bankName || "STATE BANK OF INDIA"} onChange={(e) => setSeller({ ...seller, bankName: e.target.value })} />
            </div>
            <div className="ib-bank-row">
              <label>Branch</label>
